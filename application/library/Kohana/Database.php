...
abstract class Kohana_Database {
	...
	public static function instance($name = NULL, array $config = NULL)
	{
		...
		if ( ! isset(Database::$instances[$name]))
		{
			if ($config === NULL)
			{
				...
				$config = Yaf_Registry::get('config')->get('database')->$name;
			}
	 ...
	 * @return   void
	 */
	public function create_in(array $item_list, $field_name = '')
	{
		if (!$item_list) {
			return $field_name . " IN ('') ";
		} else {
            $str = implode(',', array_filter(array_unique($item_list), function($v){if ($v !== '') return true;}));
